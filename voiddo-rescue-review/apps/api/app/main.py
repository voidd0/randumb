...
@app.get("/admin/campaign-geo-hygiene", dependencies=[Depends(require_admin)])
def campaign_geo_hygiene_get(limit: int = 500):
    return {"ok": True, "hygiene": campaign_geo_hygiene_snapshot(limit)}


@app.post("/admin/campaign-geo-hygiene/repair", dependencies=[Depends(require_admin)])
async def campaign_geo_hygiene_repair(request: Request):
    payload = await request.json()
    return {
        "ok": True,
        "hygiene": repair_campaign_geo_mismatches(
            int(payload.get("limit", 500)),
            bool(payload.get("apply", False)),
        ),
    }
