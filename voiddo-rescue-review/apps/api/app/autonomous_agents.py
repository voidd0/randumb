...
from .post_clean_activation_packet import build_post_clean_activation_packet
...
def run_agent(agent: str, payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    agents: dict[str, Callable[[], dict[str, Any]]] = {
        ...
        "post_clean_activation_packet_agent": lambda: build_post_clean_activation_packet(int(payload.get("limit", 20)), store=False),
        ...
        "outreach_queue_suppression_hygiene_agent": lambda: outreach_queue_suppression_hygiene(
            int(payload.get("limit", 100)),
            apply=bool(payload.get("apply", True)),
        ),
        "outreach_transport_block_hygiene_agent": lambda: outreach_transport_block_hygiene(
            int(payload.get("limit", 100)),
            apply=bool(payload.get("apply", True)),
        ),
...
def runtime_daily_loop_plan() -> list[tuple[str, dict[str, Any]]]:
    """Bounded no-send loop used by the production timer."""
    return [
        ("mail_throttle_agent", {}),
        ("mail_qa_agent", {}),
...
def test_daily_loop_plan() -> list[tuple[str, dict[str, Any]]]:
    return [
        ...
        ("scanner_stale_recovery_agent", {"limit": 1, "dry_run": True}),
        ("scout_run_recovery_snapshot_agent", {"limit": 1, "dry_run": True}),
        ("self_audit_agent", {}),
        ("self_closed_loop_agent", {}),
        ("self_fix_agent", {}),
        ("self_learning_agent", {}),
...
def _run_daily_loop_unlocked() -> dict[str, Any]:
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        selected = runtime_daily_loop_plan()
    runs = [run_agent(agent, payload) for agent, payload in selected]
